...
def synthesize(audio_path, face_path, outfile):
    with tempfile.TemporaryDirectory() as out_dir:
        ...
        if not os.path.isfile(face_path):
            ...
            subprocess.call(f'ffmpeg  -hide_banner -loglevel error -y -i {face_path} -vf "crop=trunc(iw/2)*2:trunc(ih/2)*2" {face_path_video}', shell=shell)
        ...
        if not audio_path.endswith('.wav'):
            ...
            command = 'ffmpeg -hide_banner -loglevel error -y -i {} -strict -2 {}'.format(audio_path, out_dir + '/temp.wav')
        ...
        command = 'ffmpeg -hide_banner -loglevel error -y -i {} -i {} -strict -2 -q:v 1 {}'.format(audio_path, out_dir + '/result.avi', outfile)
        subprocess.call(command, shell=shell)
...
def generate():
    ...
    assert "key" in data
    assert "reference_image" in data
    key = data["key"]

    if 'normalize_embedding' not in data.keys():
        data['normalize_embedding'] = False

    model = data["model_name"]
    assert model in ['speakernet-cv', 'vctk-vits']
    components = np.load(f'pca/{model}-components.npy')
    ...
    with tempfile.TemporaryDirectory() as out_dir:
        ...
        for i in range(pca_weights.shape[0]):
            ...
            stn_tst = get_text(data["text"], hps)
            with torch.no_grad():
                x_tst = stn_tst.cuda().unsqueeze(0)
                x_tst_lengths = torch.LongTensor([stn_tst.size(0)]).cuda()
                audio = net_g.infer(x_tst, x_tst_lengths, spk_emb=spk_emb, noise_scale=.667, noise_scale_w=0.8, length_scale=1)[0][
